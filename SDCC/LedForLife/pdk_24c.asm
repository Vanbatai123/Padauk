...
;--------------------------------------------------------
	...
	.globl _AT24C_writeBuf2_PARM_3
	.globl _AT24C_writeBuf2_PARM_2
	.globl _AT24C_writeBuf2_PARM_1
...
_AT24C_writeBuf_sloc4_1_0:
	.ds 1
_AT24C_writeBuf2_PARM_1:
	.ds 2
_AT24C_writeBuf2_PARM_2:
	.ds 2
_AT24C_writeBuf2_PARM_3:
	.ds 1
_AT24C_writeBuf2_sloc5_1_0:
	.ds 2
...
;	pdk_24c.c: 100: }
	ret
;	pdk_24c.c: 102: void AT24C_writeBuf2(uint16_t address, uint8_t *src, uint8_t len)
;	-----------------------------------------
;	 function AT24C_writeBuf2
;	-----------------------------------------
_AT24C_writeBuf2:
;	pdk_24c.c: 105: if ((address + len) >= MEM_BYTE)
	clear	p
	mov	a, _AT24C_writeBuf2_PARM_3+0
	push	af
	mov	a, _AT24C_writeBuf2_PARM_1+0
	mov	_AT24C_writeBuf2_sloc5_1_0+0, a
	pop	af
	push	af
	mov	a, _AT24C_writeBuf2_PARM_1+1
	mov	_AT24C_writeBuf2_sloc5_1_0+1, a
	pop	af
	add	a, _AT24C_writeBuf2_sloc5_1_0+0
	push	af
	mov	a, _AT24C_writeBuf2_sloc5_1_0+1
	addc	p, a
	pop	af
	mov	a, p
	sub	a, #0x04
	t0sn.io	f, c
	goto	00102$
;	pdk_24c.c: 107: len = (MEM_BYTE - address);
	mov	a, _AT24C_writeBuf2_PARM_1+0
	neg	a
	mov	_AT24C_writeBuf2_PARM_3+0, a
00102$:
;	pdk_24c.c: 115: I2C_beginTransmission(MEMADDR | ((address >> 8) & 0x07));
	mov	a, _AT24C_writeBuf2_PARM_1+1
	and	a, #0x07
	or	a, #0x50
	mov	_I2C_beginTransmission_PARM_1+0, a
	call	_I2C_beginTransmission
;	pdk_24c.c: 116: I2C_write(address & 0xFF);
	mov	a, _AT24C_writeBuf2_PARM_1+0
	mov	_I2C_write_PARM_1+0, a
	call	_I2C_write
;	pdk_24c.c: 122: I2C_writeBuf(src, len);
	mov	a, _AT24C_writeBuf2_PARM_2+0
	mov	_I2C_writeBuf_PARM_1+0, a
	mov	a, _AT24C_writeBuf2_PARM_2+1
	mov	_I2C_writeBuf_PARM_1+1, a
	mov	a, _AT24C_writeBuf2_PARM_3+0
	mov	_I2C_writeBuf_PARM_2+0, a
	call	_I2C_writeBuf
;	pdk_24c.c: 124: I2C_endTransmission();
	call	_I2C_endTransmission
;	pdk_24c.c: 125: _delay_ms(5);
	mov	a, #0x86
	mov	__delay_loop_16_PARM_1+0, a
	mov	a, #0x13
	mov	__delay_loop_16_PARM_1+1, a
...
;	pdk_24c.c: 126: }
	ret
;	pdk_24c.c: 128: void AT24C_readBuf(uint16_t address, uint8_t *dest, int len)
;	-----------------------------------------
;	 function AT24C_readBuf
;	-----------------------------------------
_AT24C_readBuf:
;	pdk_24c.c: 133: if ((address + len) >= MEM_BYTE)
	mov	a, _AT24C_readBuf_PARM_1+0
	mov	_AT24C_readBuf_sloc6_1_0+0, a
	mov	a, _AT24C_readBuf_PARM_1+1
	mov	_AT24C_readBuf_sloc6_1_0+1, a
	mov	a, _AT24C_readBuf_PARM_3+1
	mov	p, a
	mov	a, _AT24C_readBuf_PARM_3+0
	add	a, _AT24C_readBuf_sloc6_1_0+0
	push	af
	mov	a, _AT24C_readBuf_sloc6_1_0+1
	addc	p, a
	pop	af
	mov	a, p
	sub	a, #0x04
	t0sn.io	f, c
	goto	00102$
;	pdk_24c.c: 135: len = (MEM_BYTE - address);
	mov	a, #0x00
...
00102$:
;	pdk_24c.c: 144: I2C_beginTransmission((MEMADDR | ((address >> 8) & 0x07)));
	mov	a, _AT24C_readBuf_PARM_1+1
	and	a, #0x07
	or	a, #0x50
	...
	mov	_I2C_beginTransmission_PARM_1+0, a
	call	_I2C_beginTransmission
;	pdk_24c.c: 145: I2C_write(address & 0xFF);
	mov	a, _AT24C_readBuf_PARM_1+0
	mov	_I2C_write_PARM_1+0, a
	call	_I2C_write
;	pdk_24c.c: 146: I2C_endTransmission();
	call	_I2C_endTransmission
;	pdk_24c.c: 147: I2C_requestFrom((MEMADDR | ((address >> 8) & 0x07)), len);
	mov	a, _AT24C_readBuf_PARM_3+0
	mov	_I2C_requestFrom_PARM_2+0, a
	mov	a, _AT24C_readBuf_sloc7_1_0+0
	mov	_I2C_requestFrom_PARM_1+0, a
	call	_I2C_requestFrom
;	pdk_24c.c: 155: while (I2C_available())
	mov	a, _AT24C_readBuf_PARM_2+0
	mov	_AT24C_readBuf_sloc8_1_0+0, a
	mov	a, _AT24C_readBuf_PARM_2+1
	mov	_AT24C_readBuf_sloc8_1_0+1, a
00103$:
	call	_I2C_available
	cneqsn	a, #0x00
	goto	00106$
;	pdk_24c.c: 157: *dest++ = I2C_read();
	call	_I2C_read
	mov	p, a
	mov	a, _AT24C_readBuf_sloc8_1_0+0
	xch	a, p
	idxm	p, a
	inc	_AT24C_readBuf_sloc8_1_0+0
	addc	_AT24C_readBuf_sloc8_1_0+1
	goto	00103$
00106$:
;	pdk_24c.c: 159: }
	ret
