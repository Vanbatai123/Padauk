...
#define PxPH PBPH
// Pin definitions
#define NEO_PIN 6	 // Pin for neopixels
#define IR_PIN 5	 // Pin for IR receiver
...
#define IR_FAIL2 0xF2 // IR fail code
...
void BUTTON_init(void)
{
	...
	setb(PBDIER, BUTTON_PIN); // set BUTTON pin as input
}
...
void IR_init(void)
{
	...
	clrb(PxC, IR_PIN);
	setb(PxPH, IR_PIN);	  // set BUTTON pin as input
	setb(PBDIER, IR_PIN); // set BUTTON pin as input
...
int main(void)
{
	...
	while (1)
	{
		...
		{
			if (IR_available() == 0 && button_value == 0)
			{ //! button_press){
				command = IR_read();
				// if (command != IR_FAIL)
				// {
