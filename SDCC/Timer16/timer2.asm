...
00002$:
	...
	ceqsn	a, p
	goto	00001$
;	timer2.c: 10: uint32_t cnt = 0;
	clear	_cnt+0
	clear	_cnt+1
	clear	_cnt+2
	clear	_cnt+3
;	timer2.c: 11: uint8_t flag = 0;
	clear	_flag+0
	.area GSFINAL
...
;	delay.h: 95: }
	ret
;	timer2.c: 13: void interrupt(void) __interrupt(0)
;	-----------------------------------------
;	 function interrupt
;	-----------------------------------------
_interrupt:
	...
	mov	a, p
	push	af
;	timer2.c: 15: if( INTRQ & INTRQ_TM2 )                       //TM2 interrupt request?
	mov.io	a, __intrq
	and	a, #0x40
	cneqsn	a, #0x00
	goto	00105$
;	timer2.c: 17: INTRQ &= ~INTRQ_TM2;                        //mark TM2 interrupt request processed
	set0.io	__intrq, #6
;	timer2.c: 18: cnt++;
	inc	_cnt+0
	addc	_cnt+1
...
;	timer2.c: 25: }
	...
	pop	af
	reti
;	timer2.c: 27: unsigned char _sdcc_external_startup(void)
;	-----------------------------------------
;	 function _sdcc_external_startup
;	-----------------------------------------
__sdcc_external_startup:
;	timer2.c: 29: EASY_PDK_INIT_SYSCLOCK_8MHZ();        //use 8MHz sysclock
	mov	a, #0x34
	mov.io	__clkmd, a
;	timer2.c: 30: EASY_PDK_CALIBRATE_IHRC(F_CPU, 5000); //tune SYSCLK to 8MHz @ 4.000V
	and	a, #'R'                        
	and	a, #'C'                        
	and	a, #((1))          
	and	a, #((8000000))     
	...
	and	a, #((5000)>>8)  
	and	a, #((0x0B))           
;	timer2.c: 31: return 0;                             //perform normal initialization
;	timer2.c: 32: }
	ret	#0x00
;	timer2.c: 34: void main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	timer2.c: 37: PBC = 0xFF;
	mov	a, #0xff
	mov.io	__pbc, a
;	timer2.c: 39: TM2C = TM2C_CLK_IHRC;                         //use IHRC -> 16 Mhz
	mov	a, #0x20
	mov.io	__tm2c, a
;	timer2.c: 40: TM2S = TM2S_PRESCALE_DIV64 | TM2S_SCALE_NONE;  //no prescale, scale 2 ~> 8MHz
	mov	a, #0x60
	mov.io	__tm2s, a
;	timer2.c: 41: TM2B = 250;                                    //divide by 69 ~> 115942 Hz (apx. 115200)
	mov	a, #0xfa
	mov.io	__tm2b, a
;	timer2.c: 43: INTEN = INTEN_TM2;                            //enable TM2 interrupt, send out initial stop bits and autobaud char
	mov	a, #0x40
	mov.io	__inten, a
;	timer2.c: 44: sei();                                   //enable global interrupts
	engint
;	timer2.c: 48: while (!flag);
00101$:
	mov	a, _flag+0
	cneqsn	a, #0x00
	goto	00101$
;	timer2.c: 50: daobit(PB, 4);
	mov	a, #0x10
	xor.io	__pb, a
;	timer2.c: 52: flag = 0;
	clear	_flag+0
