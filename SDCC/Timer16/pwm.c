...
unsigned char _sdcc_external_startup(void)
{
  ...
  EASY_PDK_CALIBRATE_IHRC(8000000,5000);        //tune SYSCLK to 8MHz @ 4.000V
...
void main(void)
{
  ...
  {
    setb(PA, 7);
    for(uint32_t i=200000; i>0; i--);          //wait approx. 3s
    clrb(PA, 7);
    for(uint32_t i=200000; i>0; i--);          //wait approx. 3s
  }
}
...
#else
  PWMGCUBL = (uint8_t) PWM_MAX << 5;        // Set the PWM upper bound (lower 3 bits)
