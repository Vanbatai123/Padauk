...
;	main.c: 15: EASY_PDK_INIT_SYSCLOCK_8MHZ();		  //use 8MHz sysclock
	mov	a, #0x34
	mov.io	__clkmd, a
;	main.c: 16: EASY_PDK_CALIBRATE_IHRC(F_CPU, 4000); //tune SYSCLK to 8MHz @ 4.000V
	and	a, #'R'                        
	and	a, #'C'                        
	and	a, #((1))          
	and	a, #((8000000))     
...
;	main.c: 27: while (1)
00102$:
;	main.c: 29: puts("Hello World!");
	mov	a, #<(___str_0 + 0)
	mov	_puts_PARM_1+0, a
	mov	a, #>(___str_0 + 0x8000 + 0)
	mov	_puts_PARM_1+1, a
	call	_puts
;	main.c: 30: _delay_ms(1000);
	mov	a, #0x29
...
;	main.c: 32: }
	ret
;	main.c: 35: void interrupt(void) __interrupt(0)
;	-----------------------------------------
;	 function interrupt
;	-----------------------------------------
_interrupt:
	...
	mov	a, p
	push	af
;	main.c: 38: if (INTRQ & INTRQ_TM2)
	mov.io	a, __intrq
	and	a, #0x40
	cneqsn	a, #0x00
	goto	00103$
;	main.c: 40: INTRQ &= ~INTRQ_TM2; // Mark TM2 interrupt request processed
	set0.io	__intrq, #6
;	main.c: 42: TIM2_interrupt();
