...
#define TIMER2_ON INTEN |= INTEN_TM2;
#define TIMER2_OFF INTEN &= ~INTEN_TM2;

#define EXTERNAL_ON INTEN |= INTEN_PA4;
#define EXTERNAL_OFF INTEN &= ~INTEN_PA4;

volatile static char inbuf[UART_IN_BUF_SIZE];
...
static void TIM2_Config(void)
{
	...
	TM2B = 103;									  // Divide by 69 ~> 9600 Hz (apx. 115200)
}
...
static void idle(void)
{
	...
	// - but there is a "UART_kbhit" in this code...
...
// TIMER2 external interrupt handler
...
// PA4 external interrupt handler
void PA4_interrupt(void)
{
	TM2CT = 25;
	TIMER2_OFF;
	EXTERNAL_OFF;
