...
;	timer2.c: 20: }
	...
	pop	af
	reti
;	timer2.c: 22: unsigned char _sdcc_external_startup(void)
;	-----------------------------------------
;	 function _sdcc_external_startup
;	-----------------------------------------
__sdcc_external_startup:
;	timer2.c: 24: EASY_PDK_INIT_SYSCLOCK_8MHZ();		  // use 8MHz sysclock
	mov	a, #0x34
	mov.io	__clkmd, a
;	timer2.c: 25: EASY_PDK_CALIBRATE_IHRC(F_CPU, 5000); // tune SYSCLK to 8MHz @ 4.000V
	and	a, #'R'                        
	and	a, #'C'                        
	and	a, #((1))          
	and	a, #((8000000))     
	...
	and	a, #((5000)>>8)  
	and	a, #((0x0B))           
;	timer2.c: 26: return 0;							  // perform normal initialization
;	timer2.c: 27: }
	ret	#0x00
;	timer2.c: 29: void main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	timer2.c: 32: setb(PBC, 4);
	set1.io	__pbc, #4
;	timer2.c: 34: TM2C = TM2C_CLK_IHRC;						   // use IHRC -> 16 Mhz
	mov	a, #0x20
	mov.io	__tm2c, a
;	timer2.c: 35: TM2S = TM2S_PRESCALE_DIV64 | TM2S_SCALE_DIV5; // no prescale, scale 64 ~> 8MHz/64
	mov	a, #0x64
	mov.io	__tm2s, a
;	timer2.c: 36: TM2B = 25;									   // divide by 69 ~> 115942 Hz (apx. 115200)
	mov	a, #0x19
	mov.io	__tm2b, a
;	timer2.c: 38: INTEN = INTEN_TM2; // enable TM2 interrupt, send out initial stop bits and autobaud char
	mov	a, #0x40
	mov.io	__inten, a
;	timer2.c: 39: sei();			   // enable global interrupts
	engint
;	timer2.c: 41: while (1)
00102$:
	goto	00102$
