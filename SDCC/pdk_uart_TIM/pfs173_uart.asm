...
00002$:
	...
	ceqsn	a, p
	goto	00001$
;	pfs173_uart.c: 19: char x = 'G';
	mov	a, #0x47
	mov	_x+0, a
;	pfs173_uart.c: 21: int i = 0;
	clear	_i+0
	clear	_i+1
	.area GSFINAL
...
;	delay.h: 95: }
	ret
;	pfs173_uart.c: 12: uint8_t _sdcc_external_startup(void)
;	-----------------------------------------
;	 function _sdcc_external_startup
;	-----------------------------------------
__sdcc_external_startup:
;	pfs173_uart.c: 14: ROP = ROP_INT_SRC_PB5;
	mov	a, #0x02
	mov	__rop, a
;	pfs173_uart.c: 15: EASY_PDK_INIT_SYSCLOCK_8MHZ();		  //use 8MHz sysclock
	mov	a, #0x34
	mov	__clkmd, a
;	pfs173_uart.c: 16: EASY_PDK_CALIBRATE_IHRC(F_CPU, 5000); //tune SYSCLK to 8MHz @ 4.000V
	and	a, #'R'                        
	and	a, #'C'                        
	and	a, #((1))          
	and	a, #((8000000))     
	...
	and	a, #((5000)>>8)  
	and	a, #((0x0B))           
;	pfs173_uart.c: 17: return 0;							  //perform normal initialization
;	pfs173_uart.c: 18: }
	ret	#0x00
;	pfs173_uart.c: 22: int main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	pfs173_uart.c: 25: UART_IO_config();
	call	_UART_IO_config
;	pfs173_uart.c: 26: setb(PBC, 4);
	set1	__pbc, #4
;	pfs173_uart.c: 29: INTEGS = INTEGS_PB5_FALLING;
	mov	a, #0x02
	mov	__integs, a
;	pfs173_uart.c: 30: INTEN |= INTEN_PB5;
	mov	a, __inten
	or	a, #0x01
	mov	__inten, a
;	pfs173_uart.c: 31: UART_begin();
	call	_UART_begin
;	pfs173_uart.c: 32: INTRQ = 0;
	mov	a, #0x00
	mov	__intrq, a
;	pfs173_uart.c: 33: sei();
	engint
;	pfs173_uart.c: 35: UART_print("\rOK\r");
	mov	a, #<(___str_0 + 0)
	mov	_UART_print_PARM_1+0, a
	mov	a, #>(___str_0 + 0x8000 + 0)
	mov	_UART_print_PARM_1+1, a
	call	_UART_print
;	pfs173_uart.c: 37: while (1)
00105$:
;	pfs173_uart.c: 61: i = 0;
	clear	_i+0
	clear	_i+1
;	pfs173_uart.c: 62: while (softuart_kbhit())
00101$:
	call	_softuart_kbhit
	cneqsn	a, #0x00
	goto	00103$
;	pfs173_uart.c: 64: u[i++] = softuart_getchar();
	mov	a, _i+1
	mov	p, a
	mov	a, _i+0
	inc	_i+0
	...
	goto	00101$
00103$:
;	pfs173_uart.c: 66: u[i] = '\0';
	mov	a, #(_u + 0)
	add	a, _i+0
	mov	p, a
	mov	a, #0x00
	idxm	p, a
;	pfs173_uart.c: 67: UART_print(u);
	mov	a, #(_u + 0)
	mov	_UART_print_PARM_1+0, a
	clear	_UART_print_PARM_1+1
	call	_UART_print
;	pfs173_uart.c: 68: _delay_ms(500);
	mov	a, #0x14
...
;	pfs173_uart.c: 70: }
	ret
;	pfs173_uart.c: 73: void interrupt(void) __interrupt(0)
;	-----------------------------------------
;	 function interrupt
;	-----------------------------------------
_interrupt:
	...
	mov	a, p
	push	af
;	pfs173_uart.c: 76: if (INTRQ & INTRQ_TM2)
	mov	a, __intrq
	and	a, #0x40
	cneqsn	a, #0x00
	goto	00102$
;	pfs173_uart.c: 78: INTRQ &= ~INTRQ_TM2; // Mark TM2 interrupt request processed
	set0	__intrq, #6
;	pfs173_uart.c: 80: TIM2_interrupt();
	call	_TIM2_interrupt
00102$:
;	pfs173_uart.c: 83: if (INTRQ & INTRQ_PB5)
	mov	a, __intrq
	and	a, #0x01
	cneqsn	a, #0x00
	goto	00105$
;	pfs173_uart.c: 85: INTRQ &= ~INTRQ_PB5; // Mark TM2 interrupt request processed
	set0	__intrq, #0
;	pfs173_uart.c: 87: PB5_interrupt();
	call	_PB5_interrupt
00105$:
;	pfs173_uart.c: 89: }
	pop	af
