...
unsigned char counter = 0;	   //even 2,4,6 when TRANSMIT
unsigned char counter2 = 0;	   //even 2,4,6 when TRANSMIT
unsigned char bit_out = 0;	   //
unsigned char state = 0;		   //
unsigned char last_bit_sent = 0; //
unsigned char TX_data = 0;	   //
unsigned char RX_data = 0;	   //
unsigned char SW_UART_TX_BUFFER_FULL;
unsigned char SW_UART_RX_BUFFER_FULL;
unsigned char SW_UART_RX_BUFFER_OVERFLOW;
unsigned char SW_UART_FRAME_ERROR;

unsigned char SW_UART_status;

void SW_UART_Enable(void);
void SW_UART_Transmit(unsigned char);
...
void SW_UART_Transmit(unsigned char byte)
{
