...
#define IR_available() IR_Px					  // return 1 if IR line is low
...
void IR_init(void)
{
	IR_Px = 1;//setb(Px, IR_PIN);	  // pullup on IR pin
	IR_PxC = 0;//clrb(PxC, IR_PIN);	  // set IR pin as input
	IR_PxPH = 1;//setb(PxPH, IR_PIN);	  // set IR pin pull up
//	IR_PxDIER = 1;//setb(PxDIER, IR_PIN); // set IR pin as input
...
void IR_read(void)
{
	...
	{
		data = data << 1;
		while (IR_Px == 0 && count_ms != 100)
		{}
			;
		while (IR_Px == 1 && count_ms != 100)
		{}
			;
		count_ms1 = count_ms;
		if (count_ms1 == 100)
